...
const Recipe = require('../models/Recipe');
 ...
 exports.homepage =  async(req,res) =>{
    try{
      ...
      const latest_recipe = await Recipe.find({}).sort({_id:-1}).limit(limit_number);
      const thai = await Recipe.find({category:'Thai'}).sort({_id:-1}).limit(limit_number);
      const american = await Recipe.find({category:'Cmerican'}).sort({_id:-1}).limit(limit_number);
      const chinese = await Recipe.find({category:'Chinese'}).sort({_id:-1}).limit(limit_number);
      const indian = await  Recipe.find({category:'Indian'}).sort({_id:-1}).limit(limit_number);
      const mexican = await Recipe.find({category:'Mexican'}).sort({_id:-1}).limit(limit_number);

      const food_catagories = {thai,chinese,indian,american,mexican};
    res.render('index',{title : 'Cooking Blog - Home',categories,latest_recipe,food_catagories});
...
exports.latestRecipes =  async(req,res) =>{
   ...
   res.render('latest-recipe',{title : 'Cooking Blog - Latest Recipes',latest_recipe});
...
exports.randomRecipes =  async(req,res) =>{
   try{
      ...
      let random_recipe = await Recipe.findOne().skip(random).exec();
     
   res.render('random-recipe',{title : 'Cooking Blog - Latest Recipes',random_recipe});
...
exports.exploreSubmit = async(req,res) =>{
   ...
   res.render('submit-recipe',{title : 'Cooking Blog - Latest Recipes'});
...
exports.exploreSubmitPost = async (req,res) =>{
  try{
      let uploadfile ;   
    let uploadpath;
      let newImagename;
      ...
      if( !req.files || Object.keys(req.files).length == 0){
        ...
        newImagename = Date.now() + uploadfile;
